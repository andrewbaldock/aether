...
- **End-to-end browser tests** — there's no scripted-browser test layer today, only unit tests (vitest). Add one (Playwright is the bun-friendly choice) covering the mobile/interaction paths that keep regressing: send + stop, sidebar rename/delete on a mobile viewport, "explore further" tap, resize handle, portrait stacking, a render-tool round trip. Wire a `bun run test:e2e` script (+ optional CI); new dev dep → update `STACK.md`. Worth standing up early so each mobile fix gains a regression guard.
...
- **Shared menu/select primitive — migrate the ForceGraph node menu (remaining piece)** — most of this shipped: `@radix-ui/react-dropdown-menu` now backs the "Explore further" kebab (table/chart/images/timeline) AND the sidebar conversation kebab; `@radix-ui/react-select` backs the model picker. Still on the hand-rolled pattern: the **KnowledgeGraph ForceGraph** SVG node menu (bespoke, canvas-anchored + drag-aware). Migrate it onto the shared `ExploreMenu` for one consistent menu system.
- **Unified Canvas (project "bigsail")** — Miro-style shared canvas where capability widgets become draggable cards. The render-tool widgets (table, chart, timeline, map) are deliberately built canvas-ready (self-contained spec from `state`); dropping them onto a canvas needs zero renderer changes.
