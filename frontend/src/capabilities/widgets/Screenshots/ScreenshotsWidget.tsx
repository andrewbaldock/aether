...
export function ScreenshotsWidget(_props: { widget: Widget }) {
  ...
  return (
    // Screenshots is the deliberate oddball — it needs the full width for the
    // device grid — but routes through the shared AdminPage so its outer chrome
    // (scroll container, padding, tab-bar position, heading) matches the other
    // admin pages and the tab bar doesn't jump when navigating to it.
    <AdminPage
      title="Screenshots"
      width="max-w-5xl"
      actions={
    ...
    >
      ...
      {loadState === "missing" && (
        <p className="mt-8 text-sm text-content-muted">
          No screenshots yet. Hit <span className="text-content">Run now</span>{" "}
          (or run{" "}
          <code className="rounded bg-elevated px-1">bun run screenshots</code>{" "}
